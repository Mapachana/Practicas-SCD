...
class MonitorBarberia : public HoareMonitor{
    private:
        ...
        CondVar cliente_espera_en_sala_2;
        CondVar cliente_cortandose; // Cola para que un cliente espere mientras le cortan el pelo

    public:
        MonitorBarberia(); // Constructor
        void cortarPelo(int num_cliente, int tipo); // Cliente entra a la barberia a cortarse el pelo
...
MonitorBarberia::MonitorBarberia(){
    ...
    cliente_espera_en_sala_2 = newCondVar();
    cliente_cortandose = newCondVar();
}

// Cliente entra a la barberia a cortarse el pelo
void MonitorBarberia::cortarPelo(int num_cliente, int tipo){
    mtx.lock();
    cout << "Cliente " << num_cliente << ": entra en la barberia " << "(tipo " << tipo << ")" << endl << flush;
    mtx.unlock();

    if (!barbero_duerme.empty() && tipo == siguiente_tipo){
    ...
    else if (tipo == 0){// && (!cliente_cortandose.empty() || !cliente_espera_en_sala.empty())){
    ...
    else if (tipo == 1){//} && (!cliente_cortandose.empty() || !cliente_espera_en_sala_2.empty())){
        cliente_espera_en_sala_2.wait();
        ...
        mtx.lock();
        cout << "Cliente " << num_cliente << ": Ya me he cortado el pelo jaja" << endl << flush;
        mtx.unlock();
    }
}
...
void MonitorBarberia::siguienteCliente(){
    if (((cliente_espera_en_sala.empty() && siguiente_tipo == 0 ) || (cliente_espera_en_sala_2.empty() && siguiente_tipo == 1)) && cliente_cortandose.empty()){
        mtx.lock();
        ...
        if (siguiente_tipo == 0){
            ...
            cliente_espera_en_sala_2.signal();
        }      
    }
...
void funcionHebraCliente(MRef<MonitorBarberia> monitor, int num, int tipo){
    while(true){
        monitor->cortarPelo(num, tipo);
...
int main(){
    ...
    for (int i = 0; i < numero_clientes; ++i){
        if (i < numero_clientes/2)
            hebras_clientes[i] = thread(funcionHebraCliente, barberia, i, 0);
        else
            hebras_clientes[i] = thread(funcionHebraCliente, barberia, i, 1);
    }
