...
void MonitorBarberia::cortarPelo(int num_cliente){
    ...
    if (!barbero_duerme.empty()){
       ...
       if (cliente_espera_en_sala.get_nwt() >= numero_sillas){
           mtx.lock();
           cout << "Cliente " << num_cliente << ": Está lleno, espero en la puerta" << endl << flush;
           mtx.unlock();
