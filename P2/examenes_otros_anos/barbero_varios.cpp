...
class MonitorBarberia : public HoareMonitor{
    private:
        ...
        CondVar cliente_cortandose[numero_barberos]; // Cola para que un cliente espere mientras le cortan el pelo
...
MonitorBarberia::MonitorBarberia(){
    ...
    for (int i = 0; i < numero_barberos; ++i)
        cliente_cortandose[i] = newCondVar();
}
...
void MonitorBarberia::cortarPelo(int num_cliente){
    ...
    if (!barbero_duerme.empty()){
        ...
        cliente_cortandose[siguiente_barbero].wait();

        mtx.lock();
        cout << "Cliente " << num_cliente << ": Ya me he cortado el pelo" << endl << flush;
        mtx.unlock();
    }
    else if (!cliente_cortandose[siguiente_barbero].empty() || !cliente_espera_en_sala.empty()){
...
void MonitorBarberia::siguienteCliente(int num_barbero){
    if (cliente_espera_en_sala.empty() && cliente_cortandose[num_barbero].empty()){
        mtx.lock();
    ...
    else if (cliente_cortandose[num_barbero].empty()){
        mtx.lock();
...
void MonitorBarberia::finCliente(int num_barbero){
    ...
    cliente_cortandose[num_barbero].signal();
}
